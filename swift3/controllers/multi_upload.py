        ...
        def response_iter():
            ...
            try:
                ...
                try:
                    ...
                    msg = str(e)
                    msg_pre_260 = ('Each segment, except the last, '
                                   'must be at least ')
                    # see https://github.com/openstack/swift/commit/c0866ce
                    msg_260 = ('too small; each segment, except the last, '
                               'must be at least ')
                    # see https://github.com/openstack/swift/commit/7f636a5
                    msg_post_260 = ('too small; each segment must be '
                                    'at least 1 byte')
                ...
                SubElement(result_elem, 'Bucket').text = req.container_name
                SubElement(result_elem, 'Key').text = req.object_name
                SubElement(result_elem, 'ETag').text = '"%s-N"' % (
                    etag or put_resp.etag)
                if yielded_anything:
                    yield '\n'
